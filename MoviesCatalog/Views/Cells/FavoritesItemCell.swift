...
class FavoritesItemCell: UITableViewCell {
    ...
    func set(favorite: Favorites) {
        posterImageView.sd_setImage(with: URL(string: favorite.posterPath))
        movieTitleLabel.text = favorite.title
        movieOverviewLabel.text = favorite.overview
        releaseDateView.setupWith(date: favorite.releaseDate)
    }
